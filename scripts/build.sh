...
cd $work_dir
./gradlew build --no-daemon --refresh-dependencies -x javadoc -x integTest -Dbuild.snapshot=$SNAPSHOT -Dbuild.version_qualifier=$QUALIFIER -Dbuild.lib.commit_patches=false
