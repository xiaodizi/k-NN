...
cd $work_dir
./gradlew build --no-daemon --refresh-dependencies -x integTest -x test -Dbuild.snapshot=true -Dbuild.version_qualifier=$QUALIFIER -Dbuild.lib.commit_patches=false
...
./gradlew publishPluginZipPublicationToZipStagingRepository -Dopensearch.version=$VERSION -Dbuild.snapshot=true -Dbuild.version_qualifier=$QUALIFIER
./gradlew publishPluginZipPublicationToMavenLocal -Dbuild.snapshot=true -Dbuild.version_qualifier=$QUALIFIER -Dopensearch.version=$VERSION
