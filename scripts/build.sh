...
cd $work_dir
./gradlew build --no-daemon --refresh-dependencies -x javadoc -x integTest -x test -Dbuild.snapshot=$SNAPSHOT -Dbuild.version_qualifier=$QUALIFIER -Dbuild.lib.commit_patches=false
...
./gradlew publishPluginZipPublicationToZipStagingRepository -Dbuild.snapshot=$SNAPSHOT -Dbuild.version_qualifier=$QUALIFIER
